...
\begin{frame}
  ...
  \item Let $\mathbb P(A \cap B)$ be the joint probability of $A$ and $B$.
  ...
  \begin{block}{Splitting a joint probability}
    \[
  ...
  \begin{block}{Conditioning inversion}
    \[
...
\bigskip
   ...
   \item First manifestation of the Bayes theorem.

\bigskip

    \item The (ecact) likelihood function can be
    ...
    {\color{blue} --- Product of conditional densities,
      $y_t|y_{\underline{t-1}}$} (the conditional likelihood)\newline
...
\begin{frame}
  ...
  \item The likelihood is proportional to a Gaussian density centered on $\widehat \mu$ with variance $\nicefrac{1}{T}$.\newline

    \bigskip

  \item The posterior density:
    \[
...
\begin{frame}
  ...
  \item Previous expression can be simplified by omitting all the
...
\begin{frame}
  \frametitle{Prior and posterior beliefs}
  \framesubtitle{A simple dynamic model (cont'd, flat prior)}

      \begin{itemize}
      \item For a real parameter, the prior is proportional to 1.
        \[
        p_{0}(\varphi) \propto 1
        \]
      \item For a positive parameter, the prior of the log of the prior is proportional to 1.
        \[
        p_{0}(\log\sigma_{\epsilon}^{2}) \propto 1\quad\Leftrightarrow\quad p_{0}(\sigma_{\epsilon}^{2})\propto\frac{1}{\sigma_{\epsilon}}
        \]
      \end{itemize}

      \bigskip

      \begin{block}{Flat prior for AR(1) model}
        \[
      p_0\left(\varphi, \sigma_{\epsilon}^{2}\right) \propto \frac{1}{\sigma_{\epsilon}}
      \]
      \end{block}
      $\hookrightarrow$ Parameters $\varphi$ and $\sigma_{\epsilon}^{2}$ are a priori independent.\newline
      $\hookrightarrow$ This prior is not a pdf!
    \end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
      ...
      \item Use the conditional likelihood (omitting the marginal density of the first observation).\newline

      \item The posterior density is:
        \[
          \begin{split}
            p_{1}(\varphi,\sigma_{\epsilon}^{2}) &\propto \underbrace{\sigma_{\epsilon}^{-1}}_{\text{Prior}}
\underbrace{(2\pi)^{-\frac{T-1}{2}}\sigma_{\epsilon}^{-(T-1)}e^{-\frac{1}{2\sigma_{\epsilon}^2}\sum_{t=2}^T\left(y_t-\varphi
    y_{t-1}\right)^2}}_{\text{Conditional likelihood}} \\
&\propto
\sigma_{\epsilon}^{-T}e^{-\frac{1}{2\sigma_{\epsilon}^2}\sum_{t=2}^T\left(y_t-\varphi
    y_{t-1}\right)^2}
          \end{split}
        \]
      \end{itemize}
      ...
      \begin{block}{Posterior density}
        \[
          \begin{split}
            \varphi | \sigma_{\epsilon}^{2}, \sample  &\sim
            \normal{\frac{\sum_{t=2}^{T}y_{t}y_{t-1}}{\sum_{t=2}^{T}y_{t-1}^2} }{ \frac{\sigma_{\epsilon}^2}{\sum_{t=2}^{T}y_{t-1}^2}  }\\
            \sigma_{\epsilon}^{2} | \sample  &\sim \mathcal{IG}\left(T-2, \sum_{t=2}^T\widehat{\epsilon}_{t}^{2}\right)
          \end{split}
        \]
      \end{block}
